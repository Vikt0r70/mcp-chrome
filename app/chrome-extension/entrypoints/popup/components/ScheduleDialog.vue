<template>
  <div v-if="visible" class="rr-modal">
    <div class="rr-dialog">
      <div class="rr-header">
        <div class="title">{{ getMessage('scheduleExecutionTitle') }}</div>
        <button class="close" @click="$emit('close')">✕</button>
      </div>
      <div class="rr-body">
        <div class="row">
          <label>{{ getMessage('enabledLabel') }}</label>
          <label class="chk"
            ><input type="checkbox" v-model="enabled" />{{
              getMessage('enableScheduleLabel')
            }}</label
          >
        </div>
        <div class="row">
          <label>{{ getMessage('typeLabel') }}</label>
          <select v-model="type">
        ...
        <div class="row" v-if="type === 'interval'">
          <label>{{ getMessage('intervalMinutesLabel') }}</label>
          <input type="number" v-model.number="intervalMinutes" />
        </div>
        <div class="row" v-if="type === 'daily'">
          <label>{{ getMessage('timeHhMmLabel') }}</label>
          <input v-model="dailyTime" :placeholder="getMessage('timeHhMmPlaceholder')" />
        </div>
        <div class="row" v-if="type === 'once'">
          <label>{{ getMessage('timeIsoLabel') }}</label>
          <input v-model="onceAt" :placeholder="getMessage('timeIsoPlaceholder')" />
        </div>
        <div class="row">
          <label>{{ getMessage('argsJsonLabel') }}</label>
          <textarea v-model="argsJson" placeholder='{ "username": "xx" }'></textarea>
        </div>
        <div class="section">
          <div class="section-title">{{ getMessage('existingSchedulesLabel') }}</div>
...
function describe(s: any) {
  if (s.type === 'interval') return getMessage('scheduleIntervalDescription', [String(s.when)]);
  if (s.type === 'daily') return getMessage('scheduleDailyDescription', [String(s.when)]);
  if (s.type === 'once') return getMessage('scheduleOnceDescription', [String(s.when)]);
  return '';
}
</script>
