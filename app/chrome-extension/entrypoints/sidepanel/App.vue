    ...
    <div v-show="activeTab === 'element-markers'" class="element-markers-content">
      <div class="px-4 py-4">
        ...
        <div class="em-toolbar">
          <div class="em-search-wrapper">
            ...
            <input
              ...
              :placeholder="getMessage('markerSearchPlaceholder')"
              type="text"
            />
            <button
              v-if="markerSearch"
          ...
          <button
            ...
            :title="getMessage('elementMarkerAddTitle')"
          >
          ...
          <div class="em-modal">
            <div class="em-modal-header">
              <h3 class="em-modal-title">
                {{
                  editingMarkerId
                    ? getMessage('elementMarkerEditTitle')
                    : getMessage('elementMarkerAddTitle')
                }}
              </h3>
            ...
            <form @submit.prevent="saveMarker" class="em-form">
              <div class="em-form-row">
                <div class="em-field">
                  ...
                  <input
                    ...
                    class="em-input"
                    :placeholder="getMessage('elementMarkerNamePlaceholder')"
                    required
                  />
                </div>
              </div>
              ...
              <div class="em-form-row em-form-row-multi">
                <div class="em-field">
                  <label class="em-field-label">{{ getMessage('selectorTypeLabel') }}</label>
                ...
                <div class="em-field">
                  <label class="em-field-label">{{ getMessage('matchTypeLabel') }}</label>
                  <div class="em-select-wrapper">
                    <select v-model="markerForm.matchType" class="em-select">
                      <option value="prefix">{{ getMessage('matchTypePrefix') }}</option>
                      <option value="exact">{{ getMessage('matchTypeExact') }}</option>
                      <option value="host">{{ getMessage('matchTypeHost') }}</option>
                    </select>
                  </div>
                </div>
              </div>
              ...
              <div class="em-form-row">
                <div class="em-field">
                  <label class="em-field-label">{{ getMessage('selectorLabel') }}</label>
                  <textarea
                    v-model="markerForm.selector"
                    class="em-textarea"
                    :placeholder="getMessage('elementMarkerSelectorOrXPathPlaceholder')"
                    rows="3"
                    required
                  ></textarea>
                </div>
          ...
          <div class="em-stats-bar">
            <span class="em-stats-text">
              <template v-if="markerSearch">
                {{
                  getMessage('markerFilteredStats', [
              ...
              <template v-else>
                {{
                  getMessage('markerStats', [String(markers.length), String(groupedMarkers.length)])
                }}
              </template>
            </span>
          </div>
            ...
            <div class="em-domain-header" @click="toggleDomain(domainGroup.domain)">
              <div class="em-domain-info">
                ...
                <span class="em-domain-count">{{
                  getMessage('markerCountLabel', [String(domainGroup.count)])
                }}</span>
              </div>
            </div>
                      ...
                      <div class="em-marker-row-top">
                        ...
                        <div class="em-marker-actions">
                          <button
                            ...
                            :title="getMessage('validateButton')"
                          >
        ...
        <div v-else-if="markers.length > 0 && filteredMarkers.length === 0" class="em-empty">
          <p>{{ getMessage('noMatchingMarkers') }}</p>
        ...
        <div v-else class="em-empty">
          <p>{{ getMessage('noMarkersMessage') }}</p>
...
const groupedMarkers = computed(() => {
  ...
  for (const marker of filteredMarkers.value) {
    ...
    const domain = marker.host || getMessage('localFileLabel');
    const fullUrl = marker.url || getMessage('unknownUrlLabel');
...
function createTrigger() {
  ...
  alert(getMessage('v3TriggerCreateUnavailable'));
}

function editTrigger(_id: string) {
  // V3 Trigger management not yet implemented
  alert(getMessage('v3TriggerEditUnavailable'));
}
...
async function run(id: string) {
  try {
    ...
    if (!result) console.warn(getMessage('replayFailedLog'));
  } catch {}
}

function edit(id: string) {
  // V3 Builder not yet implemented - show message
  alert(getMessage('v3BuilderEditUnavailable'));
  // TODO: openBuilder({ flowId: id });
}

function createFlow() {
  // V3 Builder not yet implemented - show message
  alert(getMessage('v3BuilderCreateUnavailable'));
  // TODO: openBuilder({ newFlow: true });
}

async function remove(id: string) {
  try {
    const ok = confirm(getMessage('confirmDeleteWorkflowMessage'));
    if (!ok) return;
...
async function deleteMarker(marker: ElementMarker) {
  try {
    const confirmed = confirm(getMessage('confirmDeleteMarkerMessage', [marker.name]));
    if (!confirmed) return;
